...
export type DocumentType = "requirements" | "spec" | "detailed_design";
...
- 情報が不足している場合は正直に「未確認」と明記すること`,
  detailed_design: `あなたはAster Systemsの制作ディレクター（兼エンジニア）です。以下の情報（カテゴリ・ヒアリング回答・選定プラン/オプション・確定金額）だけを根拠に、
社内検討用の「詳細設計書（下書き）」を作成してください。要件定義書・仕様書よりも一段具体的な、実装に近い観点でまとめてください。
JSONのみを出力してください（前後に説明文を付けないこと）。

出力形式: {"sections":[{"heading":"見出し","body":"本文（日本語、150〜300字程度）"}, ...]}
見出しの構成例：1.画面構成（想定ページ一覧）／2.主要なデータ項目／3.処理・機能フロー／4.外部連携・使用技術の想定／5.納品物・成果物／6.実装上の留意点・未確認事項
重要な注意事項:
- 与えられた情報にない事実（存在しない機能・技術・固有名詞等）を作らないこと
- ヒアリング回答が乏しく具体的な設計に踏み込めない見出しは「ヒアリング回答からは十分な情報が得られていません。追加確認が必要です。」のように正直に書くこと
- 確定していない技術選定を断定せず、「想定」「一例として」等の表現を使うこと`,
};
